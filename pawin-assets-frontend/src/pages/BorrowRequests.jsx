...
import MainLayout from '../components/MainLayout'; // 🚀 เปลี่ยนมาใช้ MainLayout เพื่อให้เข้ากับธีม
...
export default function BorrowRequests() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axios.get('http://localhost:5000/api/borrow/pending-borrows', config);
  ...
  const handleSaveQty = async () => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/borrow/${selectedItem.id}/update-qty`, { qty: newQty }, config);
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/borrow/${selectedItem.id}/reject-borrow`, {}, config);
  ...
  return (
    <MainLayout title="รายการคำขอยืมอุปกรณ์">
