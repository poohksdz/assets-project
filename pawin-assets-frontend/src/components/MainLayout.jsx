...
import { LayoutDashboard, ShieldCheck, LogOut, Bell, Search, CheckCircle2, History, LayoutGrid, Sun, Moon, Globe, Menu, X, CheckCheck } from 'lucide-react';
...
export default function MainLayout({ children, title }) {
  ...
  // 🔌 Socket.io: รับแจ้งเตือนแบบ real-time
  useEffect(() => {
    if (!userInfo?.token) return;
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans transition-colors duration-300 selection:bg-orange-500/30 selection:text-orange-200 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-300">
      ...
      <aside className={`fixed inset-y-0 left-0 w-[280px] backdrop-blur-2xl border-r flex flex-col z-[100] transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
        } bg-white/95 dark:bg-slate-900/90 border-slate-200 dark:border-slate-800/50`}>

        <div className="absolute top-0 left-0 w-full h-32 bg-orange-500/10 blur-3xl rounded-full -z-10"></div>

        <div className="h-16 sm:h-24 flex items-center justify-between px-6 border-b transition-colors duration-300 border-slate-200 dark:border-slate-800/50">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-4 py-3 sm:py-3.5 rounded-2xl mb-2 font-bold transition-all duration-300 relative group overflow-hidden border ${isActive
                  ? 'bg-orange-100 dark:bg-gradient-to-r dark:from-orange-500/20 dark:to-transparent text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-500/30 shadow-[inset_4px_0_0_rgba(249,115,22,1)]'
                  : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-200 border-transparent'
                  }`}
              >
                {item.icon} <span className="text-sm tracking-wide">{item.title}</span>
              </Link>
            );
          })}
        ...
        <div className="p-4 sm:p-6 border-t transition-colors duration-300 border-slate-200 dark:border-slate-800/50">
          <button onClick={() => { localStorage.removeItem('userInfo'); navigate('/'); }} className="flex items-center justify-center gap-3 px-4 py-3 sm:py-3.5 w-full rounded-2xl font-black transition-all shadow-sm border bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:border-red-200 dark:hover:border-red-500/30">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-16 sm:h-24 backdrop-blur-xl flex items-center justify-between px-3 sm:px-8 z-50 border-b transition-colors duration-300 bg-white/80 dark:bg-slate-950/80 border-slate-200 dark:border-slate-800/50">
          ...
          <div className="flex items-center gap-2 sm:gap-6 ml-auto">
            ...
            {/* Search (ซ่อนในมือถือ) */}
            <div className="hidden md:flex items-center px-5 py-3 rounded-2xl w-64 lg:w-72 focus-within:border-orange-500/50 transition-all border bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 p-2 sm:p-3 rounded-xl transition-all font-black text-[10px] sm:text-xs uppercase border bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-orange-500 dark:hover:text-orange-400"
            ...
            <button
              onClick={toggleTheme}
              className="p-2 sm:p-3 rounded-xl transition-all border bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-orange-500 dark:hover:text-orange-400"
              title="Toggle Theme"
            >
            ...
            <div className="relative static sm:relative" ref={notifRef}>
              <button
                ...
                className="relative p-2 sm:p-3 rounded-xl transition-all outline-none border bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-orange-500 dark:hover:text-orange-400"
              >
                <Bell size={18} className="sm:w-5 sm:h-5" strokeWidth={2.5} />
                {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-red-500 border-2 rounded-full shadow-[0_0_10px_rgba(239,68,68,0.8)] border-white dark:border-slate-950 animate-pulse"></span>}
              </button>
              ...
              {isNotifOpen && (
                <div className="fixed inset-x-3 top-[70px] sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-4 w-auto sm:w-80 rounded-[1.5rem] sm:rounded-3xl shadow-2xl border overflow-hidden z-[1000] animate-in fade-in slide-in-from-top-4 transition-colors duration-300 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                  <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 flex justify-between items-center">
            ...
            {/* Profile Section */}
            <div className="flex items-center gap-2 sm:gap-4 pl-2 sm:pl-3 lg:pl-6 border-l cursor-pointer group transition-colors duration-300 border-slate-200 dark:border-slate-800">
